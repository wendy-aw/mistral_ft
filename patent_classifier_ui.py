...
def initialize_mistral_client():
    ...
    if not api_key:
        st.error(
            "MISTRAL API key not found. Please set the MISTRAL environment variable."
        )
        return None
                ...
                messages=[
                    {"role": "system", "content": sys_prompt},
                    {
                        ...
                        "content": user_prompt.format(
                            patent_description=processed_text
                        ),
                    },
                ],
                temperature=0,
...
def main():
    st.set_page_config(
        page_title="Patent CPC Classifier", page_icon="📋", layout="wide"
    )
    ...
    if use_finetuned:
        ft_model_id = st.sidebar.text_input(
            "Fine-tuned Model ID",
            value="",
            help="Enter your fine-tuned model id (e.g. ft:classifier:ministral-3b-latest:1945b10d:20250711:ds1:2bb5027d)",
        )
        if ft_model_id.strip():
            model_name = ft_model_id.strip()
            st.sidebar.success("Fine-tuned model selected.")
        else:
            # Fall back to selected model
            model_name = model_options[model_options.index(model_name)]
            ...
            with st.spinner("Classifying patent text..."):
                ...
                if classification_result:
                    ...
                    for class_id in classification_result:
                        description = CPC_DESCRIPTIONS.get(
                            class_id, "Description not available"
                        )
                        formatted_results.append(
                            {"CPC Code": class_id, "Description": description}
                        )
                    ...
                    st.error(
                        "Classification failed. Please check your model name and try again."
                    )
